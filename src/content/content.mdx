...
I think the above plot is fascinating because it shows you the range of names that each metric manages to capture, but we also see how certain states have names that are easier to capture. You'll also notice that this is being shown to three decimal places. These numbers are sensitive to small differences, so moving a threshold around can change the number of matches significantly.
